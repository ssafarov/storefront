...
class WC_Geolocation
{
    ...
    public static function init()
    {
        ...
        if ('geolocation' === get_option('woocommerce_default_customer_address') || apply_filters('woocommerce_geolocation_update_database_periodically',
                false)
        ) {
            add_action('woocommerce_geoip_updater', array(__CLASS__, 'update_database'));
        }
        add_filter('pre_update_option_woocommerce_default_customer_address', array(__CLASS__, 'maybe_update_database'),
            10, 2);
    }
        ...
        if (false === $external_ip_address) {
            ...
            foreach ($ip_lookup_services_keys as $service_name) {
                ...
                if (!is_wp_error($response) && $response['body']) {
                    $external_ip_address = apply_filters('woocommerce_geolocation_ip_lookup_api_response',
                        wc_clean($response['body']), $service_name);
                    break;
                }
            }
                ...
                if (!is_wp_error($response) && $response['body']) {
                    switch ($service_name) {
                        ...
                        case 'telize' :
                            ...
                            break;
                        default :
                            $country_code = apply_filters('woocommerce_geolocation_geoip_response_' . $service_name, '',
                                $response['body']);
                            break;
                    }
